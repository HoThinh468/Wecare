...
class DinnerViewModel @Inject constructor(
    ...
    fun onMealRecordItemPlusClick(mealRecordModel: MealRecordModel) = viewModelScope.launch {
        ...
        updateMealRecordUsecase.updateMealRecordQuantity(
            mDayOfMonth, mMonth, mYear, MealTypeKey.DINNER, mealRecordModel.id, quantity
        ).collect { res ->
            _uiState.update {
    ...
    fun deleteMealRecord(mealRecordModel: MealRecordModel) = viewModelScope.launch {
        ...
        deleteMealRecordUsecase.deleteMealRecord(
            mDayOfMonth, mMonth, mYear, MealTypeKey.DINNER, mealRecordModel.id
        ).collect { res ->
            _uiState.update {
                it.copy(updateMealRecordResponse = res)
            }
    ...
    private fun getDinnerMealList(dayOfMonth: Int, month: Int, year: Int) =
